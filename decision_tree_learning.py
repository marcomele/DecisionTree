class decisionTree(object):
	...
	def decision_tree_learning(examples, attributes, parent_examples = None):
		...
		for value in decisionTree.values(examples, split_att):
			...
			sub_tree = decisionTree.decision_tree_learning(exa, [att for att in attributes if att != split_att], examples)
	...
	def learn(self):
		...
		self.tree = decisionTree.decision_tree_learning(filter(lambda labeledExa: labeledExa[1] is not None and not isinstance(labeledExa[1], dict), nomissisng), self.att)
	...
	def __prune__(tree, examples, p_func, maxPchange):
		"""
			...
			maxPchange, the tuning parameter defining the threshold at which to reject the null hypothesis, thus leaving the split
		"""
